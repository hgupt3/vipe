...
class SLAMSystem:
    ...
    def __init__(
        ...
        # These will be set in _build_components
        self.droid_net = None
        self.metric_depth = None

    def _build_components(self):
        # Use cached DroidNet if available, otherwise load fresh
        if self.model_cache is not None and self.model_cache.droid_net is not None:
            self.droid_net = self.model_cache.droid_net
        else:
        ...
        self.buffer = GraphBuffer(
            ...
            device=self.device,
            fov_min_degrees=self.config.get("fov_min_degrees", None),
            fov_max_degrees=self.config.get("fov_max_degrees", None),
        )
        ...
        if self.config.keyframe_depth is not None:
            ...
            # Use cached metric depth if available
            if self.model_cache is not None and self.model_cache.metric_depth is not None:
                self.metric_depth = self.model_cache.get_metric_depth_for_slam(self.config.camera_type)
            else:
