...
class GeoCalibIntrinsicsProcessor(IntrinsicEstimationProcessor):
    def __init__(
        ...
        if model_cache is not None:
            model = model_cache.get_geocalib(is_pinhole)
            if model is None:
        ...
        estimated_fov_y = res["camera"].vfov[0].item()
        estimated_fov_degrees = np.degrees(estimated_fov_y)
        
        # Clamp FOV to min/max bounds if specified
        clamped = False
        if fov_min_degrees is not None and estimated_fov_degrees < fov_min_degrees:
            self.fov_y = np.radians(fov_min_degrees)
            clamped = True
        elif fov_max_degrees is not None and estimated_fov_degrees > fov_max_degrees:
            self.fov_y = np.radians(fov_max_degrees)
            clamped = True
        else:
            self.fov_y = estimated_fov_y
        
        if clamped:
            logger.info(f"GeoCalib FOV {estimated_fov_degrees:.1f}° clamped to {np.degrees(self.fov_y):.1f}° (bounds: {fov_min_degrees}-{fov_max_degrees}°)")
        else:
            logger.info(f"GeoCalib FOV: {estimated_fov_degrees:.1f}°")
...
class FixedFOVIntrinsicsProcessor(IntrinsicEstimationProcessor):
    """Use a fixed vertical FOV instead of estimating with GeoCalib."""
    ...
    def __init__(
        ...
        super().__init__(video_stream, gap_sec=1.0)
        self.fov_y = np.radians(fov_degrees)  # Convert to radians
        self.camera_type = camera_type
        logger.info(f"Using fixed FOV: {fov_degrees}° (vertical)")
...
class TrackAnythingProcessor(StreamProcessor):
    ...
    def __init__(
        ...
        self.mask_phrases = list(mask_phrases)  # Copy to avoid modifying original
        self.sam_run_gap = sam_run_gap
        self.add_sky = add_sky

        # Use cached tracker if available, otherwise create new one
        if model_cache is not None and model_cache.track_anything is not None:
            self.tracker = model_cache.track_anything
            # Update sam_run_gap for this video and reset state
            self.tracker.sam_run_gap = self.sam_run_gap
            self.tracker.reset()
            # Note: sky prompt already included in cached tracker if add_sky was True in config
        else:
            # Only append sky prompt when creating fresh tracker
            if self.add_sky:
...
class AdaptiveDepthProcessor(StreamProcessor):
    ...
    def __init__(
        ...
        try:
            ...
            # Use cached video depth model if available
            if model_cache is not None and model_cache.video_depth_model is not None:
                self.video_depth_model = model_cache.video_depth_model
            else:
        ...
        if model_cache is not None and model_cache.adaptive_metric_depth is not None:
            self.depth_model = model_cache.adaptive_metric_depth
        else:
            self.depth_model = make_depth_model(metric_model)
        
        if model_cache is not None and model_cache.prior_da_model is not None:
            self.prompt_model = model_cache.prior_da_model
        else:
