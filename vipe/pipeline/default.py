...
from .processors import AdaptiveDepthProcessor, FixedFOVIntrinsicsProcessor, GeoCalibIntrinsicsProcessor, TrackAnythingProcessor
...
class DefaultAnnotationPipeline(Pipeline):
    ...
    def _add_init_processors(self, video_stream: VideoStream) -> ProcessedVideoStream:
        ...
        # Use fixed FOV if specified, otherwise use GeoCalib for intrinsic estimation
        fixed_fov = self.init_cfg.get('fixed_fov_degrees', None)
        fov_min = self.init_cfg.get('fov_min_degrees', None)
        fov_max = self.init_cfg.get('fov_max_degrees', None)
        
        if fixed_fov is not None:
            # Completely bypass GeoCalib with a fixed FOV
            init_processors.append(FixedFOVIntrinsicsProcessor(
                video_stream,
                fov_degrees=fixed_fov,
                camera_type=self.camera_type
            ))
        else:
            # Use GeoCalib, optionally with min/max clamping
            init_processors.append(GeoCalibIntrinsicsProcessor(
                video_stream,
                camera_type=self.camera_type,
    ...
    def _add_post_processors(
        ...
        if (depth_align_model := self.post_cfg.depth_align_model) is not None:
            post_processors.append(AdaptiveDepthProcessor(
                slam_output, 
                view_idx, 
                depth_align_model,
                model_cache=self.model_cache,
